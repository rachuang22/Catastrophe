...
def lambda_handler(event, context):
  ...
  for record in event['Records']:
    ...
    key = record['s3']['object']['key'] # filename, in this case is also the url
    lastaccess = record['eventTime'] # the date and time of the "event", so when AWS processed the request
    ...
    with open(tmp.name, 'wb') as f:
      ...
      table.put_item(Item = { # place gathered results in the dynamoDB table
                        "url": key,
                        "timestamp": lastaccess,
                        "probability": str(prediction_prob),
                        "object": prediction_label 
                        }
                    )
  print(results)
  return results
